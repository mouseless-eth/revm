...
pub struct RefDBWrapper<'a, Error: Send + Sync> {
    pub db: &'a (dyn DatabaseRef<Error = Error> + Send + Sync),
}

impl<'a, Error: Send + Sync> RefDBWrapper<'a, Error> {
    pub fn new(db: &'a (dyn DatabaseRef<Error = Error> + Send + Sync)) -> Self {
        Self { db }
    }
}

#[async_trait::async_trait]
impl<'a, Error: Sync + Send> Database for RefDBWrapper<'a, Error> {
    type Error = Error;
    /// Get basic account information.
    async fn basic(&mut self, address: B160) -> Result<Option<AccountInfo>, Self::Error> {
